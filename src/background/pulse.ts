...
另外输出一段**整体概览**：1-2 句中文短文，给用户一个"今天该聚焦什么"的全局判断。
- 写法像跟用户面对面说话，不是写报告：例如"HSBC 入账提醒和家长会回复是今天最紧急的两件；你父亲昨天发的两封邮件还没回，建议下午集中处理。"
- 30-80 字，最多 2 句。少于 5 件事就只写 1 句。
- 不要罗列所有事项；只挑出"用户应该最先看的 1-2 件"或者"今天的 dominant theme"
- 不要空话（不要写"今天有几件事需要关注"）。要具体——指明谁、什么事、为什么紧
...
{
  "items": [ ... 排序后；元素结构和输入完全一致，id 保留 ... ],
  "overview": "1-2 句整体概览（见 system prompt 里的写法要求）"
}`;
}
...
// 极少数 / 单个事项时不值得再喂 LLM——本地拼一句概览，省一次调用
function localOverview(items: BriefingItem[]): string | null {
  if (items.length === 0) return null;
  const high = items.filter((i) => i.priority === 'high').length;
  const med = items.filter((i) => i.priority === 'medium').length;
  const low = items.filter((i) => i.priority === 'low').length;
  if (items.length === 1) {
    return `今天只有 1 件待处理：${items[0]!.title}`;
  }
  const parts: string[] = [];
  if (high) parts.push(`${high} 件高`);
  if (med) parts.push(`${med} 件中`);
  if (low) parts.push(`${low} 件低`);
  return `今天 ${items.length} 件待处理（${parts.join(' / ')}）。`;
}

export async function runBriefing(items: BriefingItem[]): Promise<BriefingResult> {
  if (items.length === 0) return { items: [], overview: null };
  if (items.length <= 2) {
    // 太少没必要再喂一次 LLM
    const sorted = [...items].sort((a, b) => priorityRank(a.priority) - priorityRank(b.priority));
    return { items: sorted, overview: localOverview(sorted) };
  }
  ...
  try {
    ...
    const parsed = tryParseJSON(out.text) as
      | { items: BriefingItem[]; overview?: string }
      | null;
    if (parsed && Array.isArray(parsed.items)) {
      ...
      const overview =
        typeof parsed.overview === 'string' && parsed.overview.trim()
          ...
          : localOverview(ordered);
      return { items: ordered, overview };
    }
  } catch (err) {
    console.warn('[ThunderClaw] briefing call failed:', err);
  }
  const fallback = [...items].sort((a, b) => priorityRank(a.priority) - priorityRank(b.priority));
  return { items: fallback, overview: localOverview(fallback) };
}
